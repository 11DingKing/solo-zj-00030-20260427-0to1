...
const WeeklyReport: React.FC = () => {
  ...
  const fetchReport = async () => {
    setLoading(true);
    try {
      const res = await weeklyReportApi.getByWeek(
        selectedTeam,
        selectedWeekStart,
      );
      const data = res.data;
  ...
  const currentWeekEnd = endOfWeek(new Date(selectedWeekStart), {
    weekStartsOn: 1,
  });
  ...
  const getTotalHours = () => {
    return workHours.reduce((total, entry) => {
      return (
        total + Object.values(entry.hours).reduce((sum, h) => sum + (h || 0), 0)
      );
    }, 0);
  };
            ...
            >
              <div>
                <h3 style={{ fontWeight: 600 }}>
                  周报周期：
                  {format(new Date(selectedWeekStart), "yyyy年MM月dd日", {
                    locale: zhCN,
                  })}{" "}
