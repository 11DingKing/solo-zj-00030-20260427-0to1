...
import { teamApi, dailyReportApi, weeklyReportApi, commentApi } from '../services/api'
import { Team, DailyReport, WeeklyReport, Comment, TeamMember } from '../types'
...
const TeamReports: React.FC = () => {
  ...
  const [reports, setReports] = useState<DailyReport[] | WeeklyReport[]>([])
  ...
  const handleViewReport = async (report: DailyReport | WeeklyReport) => {
  ...
  const handleMentionSelect = (username: string) => {
    ...
    if (lastAtIndex !== -1) {
      lines[lines.length - 1] = lastLine.slice(0, lastAtIndex + 1) + username + ' '
  ...
  const renderReportRow = (report: DailyReport | WeeklyReport, index: number) => {
    ...
    return (
      <tr key={report.id}>
        <td style={{ fontWeight: 600 }}>{report.user_name || report.username}</td>
        <td>
          {isDaily
            ? format(new Date(dailyReport.report_date), 'yyyy-MM-dd', { locale: zhCN })
        ...
        <td>
          <button className="btn btn-secondary btn-sm" onClick={() => handleViewReport(report)}>
            查看详情
          </button>
        </td>
      </tr>
    ...
    <div>
      ...
      <div className="card" style={{ marginBottom: '1.5rem' }}>
        <div className="card-body">
          <div className="grid grid-cols-2 gap-4" style={{ gridTemplateColumns: 'repeat(5, 1fr)' }}>
      ...
      <div className="card">
        <div className="card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h3 style={{ fontWeight: 600 }}>
            ...
            {reports.length > 0 && <span style={{ fontSize: '0.875rem', color: 'var(--gray-500)', fontWeight: 400 }}> ({reports.length} 条记录)</span>}
          </h3>
      ...
      {showReportDetail && selectedReport && (
        ...
        >
          <div
            className="card"
            style={{ width: '100%', maxWidth: '800px', maxHeight: '90vh', overflow: 'hidden' }}
            onClick={(e) => e.stopPropagation()}
          >
            <div className="card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <h3 style={{ fontWeight: 600 }}>
                  {selectedReport.user_name || selectedReport.username} 的{reportType === 'daily' ? '日报' : '周报'}
                </h3>
                <span className={`badge mt-2 ${selectedReport.status === 'submitted' ? 'badge-success' : 'badge-warning'}`}>
                  {selectedReport.status === 'submitted' ? '已提交' : '草稿'}
                </span>
              </div>
              <button className="btn btn-secondary btn-sm" onClick={() => setShowReportDetail(false)}>
                ✕
              </button>
            </div>
            ...
            <div style={{ maxHeight: '60vh', overflowY: 'auto' }}>
              <div className="card-body">
                {reportType === 'daily' ? (
                  <>
                    {((selectedReport as DailyReport).today_completed) && (
                      <div style={{ marginBottom: '1.5rem' }}>
                        ...
                        <div className="markdown-content" style={{ padding: '1rem', background: 'var(--gray-50)', borderRadius: 'var(--border-radius)' }}>
                          <ReactMarkdown>{(selectedReport as DailyReport).today_completed}</ReactMarkdown>
                        </div>
                      </div>
                    )}

                    {((selectedReport as DailyReport).tomorrow_plan) && (
                      <div style={{ marginBottom: '1.5rem' }}>
                        <p style={{ fontWeight: 600, marginBottom: '0.5rem' }}>明日计划：</p>
                        <div className="markdown-content" style={{ padding: '1rem', background: 'var(--gray-50)', borderRadius: 'var(--border-radius)' }}>
                          <ReactMarkdown>{(selectedReport as DailyReport).tomorrow_plan}</ReactMarkdown>
                        </div>
                      </div>
                    )}

                    {((selectedReport as DailyReport).problems) && (
                      <div>
                        <p style={{ fontWeight: 600, marginBottom: '0.5rem' }}>遇到的问题：</p>
                        <div className="markdown-content" style={{ padding: '1rem', background: 'var(--gray-50)', borderRadius: 'var(--border-radius)' }}>
                          <ReactMarkdown>{(selectedReport as DailyReport).problems}</ReactMarkdown>
                        </div>
                      </div>
                    )}
                  </>
                  ...
                  <>
                    {((selectedReport as WeeklyReport).summary) && (
                      <div style={{ marginBottom: '1.5rem' }}>
                        <p style={{ fontWeight: 600, marginBottom: '0.5rem' }}>本周总结：</p>
                        <div className="markdown-content" style={{ padding: '1rem', background: 'var(--gray-50)', borderRadius: 'var(--border-radius)' }}>
                          <ReactMarkdown>{(selectedReport as WeeklyReport).summary}</ReactMarkdown>
                        </div>
                      </div>
                    )}

                    {((selectedReport as WeeklyReport).next_week_plan) && (
                      <div style={{ marginBottom: '1.5rem' }}>
                        <p style={{ fontWeight: 600, marginBottom: '0.5rem' }}>下周计划：</p>
                        <div className="markdown-content" style={{ padding: '1rem', background: 'var(--gray-50)', borderRadius: 'var(--border-radius)' }}>
                          <ReactMarkdown>{(selectedReport as WeeklyReport).next_week_plan}</ReactMarkdown>
                        </div>
                      </div>
                    )}

                    {((selectedReport as WeeklyReport).coordination_needed) && (
                      <div style={{ marginBottom: '1.5rem' }}>
                        <p style={{ fontWeight: 600, marginBottom: '0.5rem' }}>需要协调的事项：</p>
                        <div className="markdown-content" style={{ padding: '1rem', background: 'var(--gray-50)', borderRadius: 'var(--border-radius)' }}>
                          <ReactMarkdown>{(selectedReport as WeeklyReport).coordination_needed}</ReactMarkdown>
                        </div>
                      </div>
                    )}

                    {((selectedReport as WeeklyReport).work_hours && (selectedReport as WeeklyReport).work_hours!.length > 0) && (
                      <div>
                        <p style={{ fontWeight: 600, marginBottom: '0.5rem' }}>工时统计：</p>
                        <table className="work-hours-table">
                          ...
                          <tbody>
                            {(selectedReport as WeeklyReport).work_hours!.map((entry) => (
              ...
              <div style={{ borderTop: '1px solid var(--gray-200)' }}>
                <div className="card-body" style={{ paddingBottom: '0.5rem' }}>
                  <h4 style={{ fontWeight: 600, marginBottom: '1rem' }}>评论 ({comments.length})</h4>
                </div>
                    ...
                    comments.map((comment) => (
                      <div key={comment.id} className="comment">
                        <div className="comment-header">
                          <div className="avatar">{comment.user_name?.[0] || comment.username?.[0] || 'U'}</div>
                          <div>
                            <span className="comment-author">{comment.user_name || comment.username}</span>
                        ...
                        <div className="comment-content">
                          {comment.content.replace(/@(\w+)/g, '<span class="mention">@$1</span>')}
                        </div>
                      </div>
                    ))
                  )}
                  ...
                  >
                    {filteredMembers.map((member) => (
                      ...
                      >
                        ...
                        <div style={{ fontSize: '0.75rem', color: 'var(--gray-500)' }}>@{member.username}</div>
                      </div>
                    ))}
                  </div>
                )}
